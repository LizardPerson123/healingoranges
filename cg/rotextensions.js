...
let chat = new RottingExtension({
  name: "chat",
  register: function(username, msg) {
    ...
    else if (msg.includes("<") || msg.includes(">")) {msg = "Blocked For Security Reasons"}
    else if (msg.length > 99) {msg = "Blocked For Length"}

    getById("chatShow").innerHTML += `<p style='font-size: 2em; margin-top: 5px'>${username}: ${msg}</p>`
    this.numberOfMessages++
    if (this.numberOfMessages > 10) {
      this.numberOfMessages = 10
      getById("chatShow").querySelector("p").remove()
    }
  },

  show: function() {RottingExtension.setCurrentView(this.name)},

  additonalConstuctor: function() {this.numberOfMessages = 0}
})
...
let achi = new RottingExtension({
  ...
  register: async function(acheName, type="bronze") {
    ...
    getById("achiImg" + code).src = "../" + type + "_trophy.png"
